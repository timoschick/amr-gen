...
/**
 * This class implements a maximum entropy model for articles. See {@link StanfordMaxentModelImplementation} for further details on the
 * implemented methods. The features used by {@link NumberMaxentModel#toDatumList(Amr, Vertex, boolean)} are explained in the thesis.
 */
public class NumberMaxentModel extends StanfordMaxentModelImplementation {
    ...
    public List<Datum<String, String>> toDatumList(Amr amr, Vertex vertex, boolean forTesting) {
        ...
        if(!forTesting) {
            if (!PosHelper.mapPos(vertex.getPos(), true).equals("NN")) return Collections.emptyList();
            if (instanceEdge == null || vertex.isDeleted() || vertex.isLink())
                return Collections.emptyList();
        }

        String result = GoldSyntacticAnnotations.getGoldNumber(amr, vertex);
        ...
        int depth = vertex.subtreeSize();
        String depthString = Math.min(3, vertex.subtreeSize())+"";

        List<IndicatorFeature> features = new ArrayList<>();

        features.add(new StringFeature("toDepth", depth));
        ...
        features.add(new ListFeature("outLabel-posTag", outEdges.stream().map(e -> e.getLabel() + (e.getTo().isPropbankEntry()?":PROP":e.getTo().getPos())).collect(Collectors.toList())));
        features.add(new ListFeature("outLabel", outEdges.stream().map(e -> e.getLabel()).collect(Collectors.toList())));
        features.add(new ListFeature("outLabel-instance", outEdges.stream().map(e -> e.getLabel() + instance).collect(Collectors.toList())));

        features.add(new StringFeature("toDepthString-instance", depthString + instance));
        features.add(new StringFeature("parentInst-instance", parentInstance + instance));
        ...
        // features.add(new ListFeature("wordNetHierarchy", wordnetHierarchy));
        features.add(new StringFeature("parentInst", parentInstance));
    ...
    public void applyModification(Amr amr, Vertex vertex, List<Prediction> predictions) {
        if(forcePlural(vertex)) vertex.predictions.put("number", Collections.singletonList(new Prediction(GoldSyntacticAnnotations.PLURAL, 1)));
        else vertex.predictions.put("number", predictions);
    }

    public static boolean forcePlural(Vertex v) {

        for(Edge e: v.getOutgoingEdges()) {
            if(e.isInstanceEdge()) continue;
            if(e.getLabel().equals(":quant") && !e.getTo().getInstance().equals("1")) return true;
            if(e.getLabel().equals(":mod") && e.getTo().getInstance().equals("various")) return true;
            if(e.getLabel().equals(":domain") && e.getTo().getInstance().equals("they")) return true;
        }
